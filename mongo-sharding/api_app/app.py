...
async def root():
    ...
    # helloDoc collection count per shard
    hd_stats = await db.command('collStats', 'helloDoc')
    shard_stats = {
        'shard1': f"{hd_stats['shards']['shard1']['count']} documents",
        'shard2': f"{hd_stats['shards']['shard2']['count']} documents"
    }
